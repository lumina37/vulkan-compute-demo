...
std::expected<std::vector<std::tuple<TRes...>>, Error> PerfQueryPoolBox::getResults() const noexcept {
    ...
    const auto queryRes =
        device.getQueryPoolResults(queryPool_, 0, queryIndex_, results.size() * sizeof(VkPerformanceCounterResultKHR),
                                   rawResults.data(), perfCounterCount_ * sizeof(VkPerformanceCounterResultKHR),
...
#    include "vkc/query_pool/perf.cpp"
#endif
