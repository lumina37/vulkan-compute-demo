...
#include "spirv/gaussFilterImgload.hpp"
#include "vkc.hpp"
...
int main(int argc, char** argv) {
    ...
    std::vector descPoolSizes = genPoolSizes(srcImageMgr, dstImageMgr, gaussKernelWeightsMgr);
    vkc::DescPoolManager descPoolMgr{deviceMgr, descPoolSizes};

    std::array gaussDLayoutBindings = genDescSetLayoutBindings(srcImageMgr, dstImageMgr, gaussKernelWeightsMgr);
    ...
    gaussDescSetMgr.updateDescSets(srcImageMgr, dstImageMgr, gaussKernelWeightsMgr);

    // Pipeline
    constexpr vkc::BlockSize blockSize{16, 16, 1};
    vkc::ShaderManager gaussShaderMgr{deviceMgr, shader::gaussFilterImgloadSpirvCode};
